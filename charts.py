...
def plot_pie_chart(currency_data, currency, theme):
    if theme == "dark":
...
def plot_line_chart(currency_data, currency, theme):
    if theme == "dark":
        plt.style.use('dark_background')
    else:
        plt.style.use('default')

    currency_data = currency_data.sort_values('time')
    running_balance = []
    balance = 0
    ...
    for _, row in currency_data.iterrows():
        try:
            ...
            if row['operation'] == 'WITH':
                balance += amount
            elif row['operation'] == 'DEPO':
                balance -= amount
    ...
    currency_data.plot(x='time', y='balance', ax=ax2, kind='line')
    ...
    try:
        ...
        for currency in currencies:  # Loop through each currency
            try:
                ...
                plot_pie_chart(currency_data, currency, theme)
                plot_line_chart(currency_data, currency, theme)
